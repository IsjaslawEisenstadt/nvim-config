return {
					...
					mappings = {
						...
						["l"] = function(state)
							...
							if node.type ~= "directory" or node:is_expanded() then
								require('neo-tree.ui.renderer').focus_node(state, nil, true, 1, nil)
							else
								node:expand()
							end
							...
							-- 	return
							-- end
							-- require('neo-tree.sources.common.commands').expand_all_nodes(state, node, nil)
							-- require('neo-tree.sources.common.commands').toggle_directory(state, node, nil)
	...
	},
	{
		...
		dependencies = {
			'nvim-lua/plenary.nvim',
			-- makes sure that this loads after Neo-tree.
			'nvim-neo-tree/neo-tree.nvim',
		},
		config = function()
