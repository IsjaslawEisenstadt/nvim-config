...
local function setup_buffer_list()
	bl_buf = vim.api.nvim_create_buf(false, true)

	vim.bo[bl_buf].buftype = 'nofile'
	vim.bo[bl_buf].bufhidden = 'wipe'
	vim.bo[bl_buf].swapfile = false

	local NuiLine = require 'nui.line'

	local line1 = NuiLine()
	line1:append 'autocmds.lua'
	line1:render(bl_buf, -1, 1)

	-- local line2 = NuiLine()
	-- line2:append 'main.c'
	-- line2:render(bl_buf, -1, 2)
	...
	vim.api.nvim_create_autocmd('User', {
		...
		callback = function()
			vim.print 'yes...!'
			for _, win in ipairs(vim.api.nvim_list_wins()) do
				if vim.api.nvim_win_get_buf(win) == bl_buf then
	...
	vim.api.nvim_create_autocmd('BufWinEnter', {
		callback = function(args)
			...
			if target_win and vim.api.nvim_win_is_valid(target_win) and vim.api.nvim_buf_is_valid(bufnr) then
				...
				if buf_type == 'headers' then
					vim.cmd('buffer ' .. bl_buf)
				end
			end
